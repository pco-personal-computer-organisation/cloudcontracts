...
import template from './bericht-vertrag-mit-assets.template.html';

class BerichtVertragMitAssetsCtrl {
  constructor(ImportExport, NgTableParams, $location, FileSaver, Blob) {
    Object.assign(this, {
      ImportExport,
      NgTableParams,
      $location,
  ...
  getData(params) {
    ...
    return this.ImportExport.assets().$promise
  ...
  export() {
    const sheetName = 'Vertragsdaten-mit-Assets';
    const filename = `Vertragsdaten-mit-Assets-${moment().format('YYYY-MM-DD')}.xlsx`;
...
angular.module('berichtVertragMitAssets')
  .component('berichtVertragMitAssets', {
    template,
    controller: BerichtVertragMitAssetsCtrl,
  });
