...
import template from './bericht-vertrag-mit-slas.template.html';

class BerichtVertragMitSlasCtrl {
  constructor($filter, ImportExport, NgTableParams, $location, FileSaver, Blob) {
    Object.assign(this, {
      $filter,
      ImportExport,
      NgTableParams,
  ...
  getData(params) {
    ...
    return this.ImportExport.slas().$promise
  ...
  export() {
    const sheetName = 'Vertragsdaten-mit-SLAs';
    const filename = `Vertragsdaten-mit-SLAs-${moment().format('YYYY-MM-DD')}.xlsx`;
...
angular.module('berichtVertragMitSlas')
  .component('berichtVertragMitSlas', {
    template,
    controller: BerichtVertragMitSlasCtrl,
  });
